...
const EditRelease = ({ isOpen, callClose, callParentUpdate, release }) => {
    ...
    useEffect(() => {
        if (release) {
            setFormValues({
                ...
                previewImg: release.preview.image,
                previewVid: release.preview.video,
                patreonUrl: release.patreonUrl,
                animator: creators.filter(c => c.name === release.creator.name)[0].name,
                va: creators.filter(c => c.name === release.va.name)[0].name,
                download: "",
                upscale: release.upscaleAvailable
            })
            setDownloadList(release.downloads)
        }
    }, [release])
